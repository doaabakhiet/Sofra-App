...
function sendNotification( $notification, $send_data)
{
  $token = $notification->user->tokens()->where('token', '!=', '')->pluck('token')->toArray();
  // $token = Token::where('token', '!=', '')->where('user_id', $user->id)->where('user_type',$user_type)->pluck('token')->toArray();
  if (count($token)) {
    $content = [
      'ar' => request()->user()->name . "  يوجد اشعار من",
      'en' => "You Have new Notification" . request()->user()->name,
    ];
    $title = $notification->title;
    $content = $notification->content;
    $data = [
      'send_data_id' => $send_data->id
    ];
